package com.joj.jojbackenduserservice.service.impl;
...
import com.joj.jojbackendmodel.model.dto.user.UserQueryRequest;
...
import com.joj.jojbackenduserservice.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if (currentUser == null || currentUser.getId() == null) {
            log.info("Session里显示未登录吗？？？");
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
        }
        // 从数据库查询（追求性能的话可以注释，直接走缓存）
        long userId = currentUser.getId();
        currentUser = this.getById(userId);
        log.info("userId = {}, currentUser = {}", userId, currentUser);
        if (currentUser == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
        }
        return currentUser;
